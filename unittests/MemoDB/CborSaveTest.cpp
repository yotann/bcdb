...
TEST(CborSaveTest, Float) {
  auto check = [](double value, const std::vector<uint8_t> &expected) {
    std::vector<uint8_t> out;
    memodb_value(value).save_cbor(out);
    EXPECT_EQ(expected, out);
  };
...
TEST(CborSaveTest, MapOrdering) {
  ...
  test_save(memodb_value::map({
            ...
            {
                0xa7, 0x00, 0xf7, 0x01, 0xf7, 0x18, 0x18,
                0xf7, 0x19, 0x01, 0x00, 0xf7, 0x20, 0xf7,
                0x38, 0x18, 0xf7, 0x39, 0x01, 0x00, 0xf7,
            });
  ...
  test_save(memodb_value::map({
            ...
            }),
            {
                ...
                0x80, 0xf7, 0xa0, 0xf7, 0xd8, 0x27, 0x60, 0xf7, 0xf4,
                0xf7, 0xf5, 0xf7, 0xf6, 0xf7, 0xf7, 0xf7,
            });

  // test example from RFC 8949
  test_save(memodb_value::map({
                {10, 1},
                {100, 2},
                {-1, 3},
                {"z", 4},
                {"aa", 5},
                {memodb_value::array({100}), 6},
                {memodb_value::array({-1}), 7},
                {false, 8},
            }),
            {
                0xa8, 0x0a, 0x01, 0x18, 0x64, 0x02, 0x20, 0x03,
                0x61, 0x7a, 0x04, 0x62, 0x61, 0x61, 0x05, 0x81,
                0x18, 0x64, 0x06, 0x81, 0x20, 0x07, 0xf4, 0x08,
            });
}
