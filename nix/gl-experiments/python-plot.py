...
pt = pd.pivot_table(df, values='time_mean', columns=['configuration'], index=['benchmark'])
print(pt['lto'])
...
if False:
    ...
    markerline, stemlines, baseline = ax.stem(pt.index, pt['speedup'], use_line_collection=True, bottom=1, basefmt='C7-', orientation='horizontal')
    ...
    ax.annotate(f'Geometric mean: {100*avg-100:.1f}% speedup', (avg, 35), rotation=-90, xytext=(3, 14), textcoords='offset points', color='#228822')
else:
