...
def get_clang_cmdline(exe_path):
    p = subprocess.run(('nix', 'eval', '--raw', '-f', 'clang-cmdline.nix',
        '--argstr', 'exepath', exe_path, 'args'), capture_output = True, text =
        True)
    p.check_returncode()
    cmdline = p.stdout
...
def generate_args_env(exe_path, benchmark, out, affinity):
    ...
    args = ['pyperf', 'command', '-p20', '-n1', '-w1',
            ...
            '--inherit-environ', 'LD_DEBUG,LD_DEBUG_OUTPUT,LD_BIND_NOW,PYTHONPATH',
