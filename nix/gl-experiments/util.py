...
def choose_perf_events():
    ...
    perf_events += ['cycles:u', 'cpu-cycles:u', 'stalled-cycles-frontend:u', 'stalled-cycles-backend:u', 'instructions:u']
    ...
    p = subprocess.run(('perf', 'list', '--no-desc'), capture_output = True, universal_newlines = True)
...
def run_benchmarks(generate_args_env, exe_paths, benchmarks, output_dir, cpus_per_task = 1):
    ...
    with concurrent.futures.ThreadPoolExecutor(cpu_queue.qsize()+1) as executor:
        executor.map(runner, [(exe_path, benchmark) for exe_path in exe_paths for benchmark in benchmarks])
