...
public:
  ...
  size_t getNumBlockInfos() const { return BlockInfoRecords.size(); }

  const BlockInfo *getBlockInfoByIndex(size_t i) const {
    return &BlockInfoRecords[i];
  }
};
...
class BitstreamCursor : SimpleBitstreamCursor {
  ...
  struct Block {
    ...
    explicit Block(unsigned PCS, unsigned PBID)
        : PrevCodeSize(PCS), PrevBlockID(PBID) {}
  };
...
public:
  ...
  /// Return the block ID of the current block.
  unsigned getBlockID() const { return CurBlockID; }
...
private:
  void popBlockScope() {
    ...
    CurBlockID = BlockScope.back().PrevBlockID;
