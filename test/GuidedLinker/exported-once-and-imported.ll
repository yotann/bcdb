...
; RUN: bcdb add -store sqlite:%t.bcdb %p/Inputs/exported-once-and-imported.ll -name import
; RUN: bcdb gl -store sqlite:%t.bcdb prog import -o %t --merged-name=libmerged.so --weak-name=libweak.so --noplugin
...
; MERGED: @exported_constant = extern_weak constant i32
