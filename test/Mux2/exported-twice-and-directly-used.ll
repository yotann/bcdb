...
; RUN: bcdb add -uri sqlite:%t.bcdb %p/Inputs/exported-twice-and-directly-used.ll -name weak
; RUN: bcdb mux2 -uri sqlite:%t.bcdb prog weak -o %t --muxed-name=libmuxed.so --weak-name=libweak.so
...
; PROG-NEXT: %1 = tail call i32 @__bcdb_id_1()
; PROG: define i32 @user()
; PROG-NEXT: call i32 @__bcdb_id_2()
